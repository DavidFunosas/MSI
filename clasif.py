...
# molecule object containing upper membrane (homogenea original, sin remove-insert)
# molecule object containing lower membrane (homogenea original, sin remove-insert)
# Esto lo crea en la linea 265
# Despues de esa linea hace todos los cambios de remove-insert para conseguir la heterogenea, el resultado de esos cambos lo guarda parece en 'mol' o en 
# 'leaflet_copy'. Eso es lo que deberia guardar en el PDB, no leaflets. El problema es que si visualizo mol o leaflets_copy las mebranas tienen residuos 
# insertados que se solapan, osea como si no hibiese hecho el remove...
# no se por que es eso. Habria que esperar a David porque yo no consigo entender bien como funcionan las funciones.
# Por si quereis visualizar lo que digo he dejado la siguientes lineas. Habreis de meter "-v" sin valor, como tag en la linea de comandos. Simplemente he
# metido un tag opcional por si el usuario quiere visualizar, y puesto raw_input()
